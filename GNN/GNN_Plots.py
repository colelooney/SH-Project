...
data = np.load('../graphdata/gnn_discriminant_scores_validation_29th_september_low_lr.npz')
...
def main():
    ...
    plt.title('GNN Validation Discriminant Scores Weighted by Lumi Weight')
    plt.xlabel('Discriminant Score (p(+) - p(-))')
    plt.ylabel('Weighted Event Count')
    plt.xlim(-1,1)
    plt.grid(False)
    plt.savefig('../plots/GNN_Discriminant_vs_LumiWeight_validation_29th_September_lowlr.png')
    plt.show()
